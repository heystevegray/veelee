...
const Converter = () => {
  ...
  const converter = converters.find(
    (converter) => converter.href === params.converter
  );
  ...
  if (!params?.converter || !converter) {
    return (
      <HeaderSection
        icon={<AlertCircle />}
        title="Unsupported converter"
        description="Sorry, this converter is not supported yet we're not Bezos. Choose from one we do support below."
      >
        <Converters />
      </HeaderSection>
    );
      ...
      <Container className="flex flex-col gap-8 pt-8 md:pt-24">
        {videoUrl ? (
          ...
          <p className="text-center">
            Add a {converter.title} url below to get started.
          </p>
        )}
      </Container>
      <ActionsToolbar>
